...
gasdata %>% 
  ggplot(aes(x=Date,y=x_ppm,col = factor(depth)))+
...
gasdata %>%
  ggplot(aes(x=depth,y=x_ppm,col = season(Date)))+
...
df <- data.frame(SAMPLE_NO = 1,
                 gas = c("N2","O2","Ar","CO2"),
                 x_ppm = c(0.78158,0.209,0.009,0.00042))
df$x_ppm <- df$x_ppm*1e6

# imitate 20 % mass difference
df$x_ppm <- 0.8*df$x_ppm
...
#### offset_correction() & recalibrate()

The atmospheric concentration of gases can be used as a proxy to 'calibrate' time series after the fact. This could be caused by a badly calibrated chromatography setup or drifts therein. There are two functions implemented in ConFluxPro that can help mitigate such effects that both use the atmospheric concentration of gases as a means to re-calibrate the gas measurements.

For long-term trends in `gasdata`, the function `offset_correction` can be used. Using the helper-function `offset_subsetting`, the data is subdivided into sections based on their Date. For each sections a `mode` of correction can be set. This must be one of "const" or "lin". In `offset_correction`, the `gases` to be corrected, their assumed concentration in `gases_std`, as well as the `depth_cat` of the atmospheric concentrations must be provided. Then for each section, the concentration data is corrected following the mode provided in `offset_subsetting`. For "constant", the `gases_std` concentration is divided by the median value of all atmospheric measurements. For "lin" it is divided by the prediction of a linear model against time of the same data. This factor is then multiplied by _all_ the concentration measurements in the relevant section.
...
In the following, datasets of soil water content `SWC`, soil total pore space `TPS` and fitting parameters of diffusion models `a` and `b`, temperature `t` and pressure `p` will be discretised to match a common depth resolution using the function `discretize_depth()`. The goal is to have a dataset, where each soil profile is divided into layers with no gaps in between and for each layer the different parameters are discretised.
...
But let's untangle what's happening in the background. soilphys_complete will create the columns `AFPS`, `DSD0`,`D0`, `DS`, and `c_air`. `AFPS` is the air filled pore space and is calculated as the difference between the total pore space `TPS` and the soil water content `SWC`.
$$
AFPS = TPS - SWC
$$
...
$$
D_0 = a\cdot ((t+273.15) / 273.15)^b \cdot 1013/p
$$

With t given in _°C_ and p in _hPa_. 
...
Finally, the number density of air `c_air` is calculated in mol m^-3^ as 
$$
\c_{air} = 100*p/(8.314\cdot (t+ 273.15))
$$
...
$$
f = dc_{ppm}/dz \cdot c_{air} \cdot D_S 
$$
...
Note that you need to define the parameters `param` that should be averaged for each layer. Obligatory are `DS` and `c_air`, as these are needed in the flux calculation.
